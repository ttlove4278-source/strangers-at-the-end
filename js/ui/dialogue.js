const DialogueSystem = {
  ...
  init() {
    ...
    this.els.screen.addEventListener('click', this._inputHandler);
  },
  ...
  showChoices(node) {
    ...
    node.choices.forEach((choice, i) => {
      ...
      item.innerHTML = `
        ...
        <span class="choice-effect">${choice.tag ? '[' + choice.tag + ']' : ''}</span>
      `;
      item.addEventListener('click', () => this.selectChoice(choice, i, node));
      item.addEventListener('mouseenter', () => {
        choiceIdx = i;
        this.highlightChoiceItems(i);
        audio.playSelect();
      });
    ...
    this.highlightChoiceItems(0);

    this._choiceHandler = (e) => {
      if (e.code === 'ArrowUp' || e.code === 'KeyW') {
        e.preventDefault();
        choiceIdx = Math.max(0, choiceIdx - 1);
        this.highlightChoiceItems(choiceIdx);
        audio.playSelect();
  ...
  startBattle(node) {
    ...
    this.els.narration.classList.remove('active');
    audio.stopCicadas();
    Engine.startBattle(node.enemy);
  },

  gotoScene(targetId) {
    const target = SCRIPT_PROLOGUE.scenes.find(s => s.id === targetId);
    if (target) {
      Transitions.fadeToBlack(350).then(() => {
        ...
        Transitions.fadeFromBlack(350).then(() => {
  ...
  skipTyping() {
    ...
    const c = target.querySelector('.typing-cursor');
    if (c) c.remove();
    this.waitingForInput = true;
    this.els.indicator.classList.add('visible');
  },
